...
function pre_install(){
	...
	[ -z "$gostpwd" ] && gostpwd="748596123"
	echo
	if ! [[ $gostpwd =~ ^[a-zA-Z0-9]+$ ]];then
	echo -e "\033[44;31m 密码里混进了奇怪的字符！密码改为默认密码748596123。请在脚本之后完毕后用“gost set”命令重新设置密码！\033[0m"
	gostpwd="748596123"
	fi
    echo "---------------------------"
	...
	if [ "$xzy" == "2" ];then
	    ...
	    xzy="http://vip.18cm.bid/ssh/gost_2.3_linux_amd64.tar.gz"
	fi
